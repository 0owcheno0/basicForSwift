...
class SWQRCodeViewController: RootChildViewController {
    ...
    private func _setupUI() {
        ...
        let albumItem = UIBarButtonItem(title: .localized_album, style: .plain, target: self, action: #selector(showAlbum))
...
// MARK: - 跳转相册
    ...
    lazy var scannerView:SWScannerView = {
        let tempScannerView = SWScannerView(frame: view.frame, config: config)
...
extension SWQRCodeViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private func handlePickInfo(_ info: [String : Any]) -> Bool {
        if let pickImage = info[UIImagePickerController.InfoKey.originalImage.rawValue] as? UIImage {
